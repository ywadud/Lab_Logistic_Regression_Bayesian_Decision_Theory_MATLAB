...
function [cost_val, grad] = lr_cost_function_reg(X, y, lambda, theta)
    ...
    % Cost
    %cost_val = (-1/m)*sum(y.*log(w) + (1-y).*log(1 - w))+ lambda/(2*m)*sum(theta(2:end).^2);
    cost_val = (-1/m)*sum(y.*log(w) + (1-y).*log(1 - w))+ lambda/(m)*sum(theta(2:end));
